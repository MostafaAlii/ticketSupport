...
class Callcenter extends Authenticatable implements JWTSubject  {
    ...
    public function isActiveManager(): Attribute {
        $managers = DB::table('callcenters')->where('status', 'active')->where('type', 'manager')->pluck('email')->toArray();
        return Attribute::make(get: fn() => in_array($this->email, $managers));
    }
